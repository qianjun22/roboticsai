"""Motion Capture Integrator — FastAPI port 9526"""
import math, random
...
def build_html():
    ...
    return f"""<!DOCTYPE html><html><head><title>Motion Capture Integrator — Port {PORT}</title>
<style>body{{margin:0;background:#0f172a;color:#e2e8f0;font-family:monospace}}
h1{{color:#C74634;padding:20px}}svg{{display:block;margin:20px}}</style></head>
<body><h1>Motion Capture Integrator — Port {PORT}</h1>
...
if USE_FASTAPI:
    app = FastAPI(title="Motion Capture Integrator")
