"""OCI Networking Optimizer — FastAPI port 8989"""
import math, random
...
def build_html():
    ...
    return f"""<!DOCTYPE html><html><head><title>OCI Networking Optimizer</title>
...
<body><h1>OCI Networking Optimizer</h1>
<div class="card"><h2>Network Throughput</h2>
<svg width="450" height="180">{bars}</svg>
<p>Current value: {data[-1]} | Peak: {max(data)} | Port: {PORT}</p>
</div></body></html>"""

if USE_FASTAPI:
    app = FastAPI(title="OCI Networking Optimizer")
