"""Sim-to-Real Domain Randomization V3 — FastAPI port 9912"""
import math, random
...
def build_html():
    ...
    return f"""<!DOCTYPE html><html><head><title>Sim-to-Real Domain Randomization V3 — Port {PORT}</title>
<style>body{{margin:0;background:#0f172a;color:#e2e8f0;font-family:monospace}}
h1{{color:#C74634;padding:20px}}svg{{display:block;margin:20px}}</style></head>
<body><h1>Sim-to-Real Domain Randomization V3 — Port {PORT}</h1>
...
if USE_FASTAPI:
    app = FastAPI(title="Sim-to-Real Domain Randomization V3")
