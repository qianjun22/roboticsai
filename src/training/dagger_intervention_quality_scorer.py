"""DAgger Intervention Quality Scorer — FastAPI port 9002"""
import math, random
...
def build_html():
    ...
    return f"""<!DOCTYPE html><html><head><title>DAgger Intervention Quality Scorer</title>
...
<body><h1>DAgger Intervention Quality Scorer</h1>
<div class="card"><h2>Intervention Scores</h2>
<svg width="450" height="180">{bars}</svg>
<p>Current value: {data[-1]} | Peak: {max(data)} | Port: {PORT}</p>
</div></body></html>"""

if USE_FASTAPI:
    app = FastAPI(title="DAgger Intervention Quality Scorer")
