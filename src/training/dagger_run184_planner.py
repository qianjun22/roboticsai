"""
DAgger run184 planner: safe exploration DAgger with constrained policy updates.
...
PORT = 10274
SERVICE = "dagger_run184_planner"
DESCRIPTION = "DAgger run184: safe exploration DAgger — constrained policy updates with zero safety violations"
...
<div class="metric"><h2>Service Info</h2><p>Port: {PORT} | Status: operational</p></div>
    ...
    @app.get("/dagger/run184/config")
    def dagger_run184_config():
        return {
            "run": 184,
            "strategy": "safe_exploration_dagger",
            "kl_constraint": 0.1,
            "safety_violations_allowed": 0,
            "sr_target": 0.93
        }

    @app.get("/dagger/run184/metrics")
    def dagger_run184_metrics():
        return {
