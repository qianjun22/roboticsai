"""Adaptive Noise Injection — FastAPI port 9158"""
import math, random
...
def build_html():
    ...
    return f"""<!DOCTYPE html><html><head><title>Adaptive Noise Injection — Port 9158</title>
<style>body{{margin:0;background:#0f172a;color:#e2e8f0;font-family:monospace}}
h1{{color:#C74634;padding:20px}}svg{{display:block;margin:20px}}</style></head>
<body><h1>Adaptive Noise Injection — Port 9158</h1>
<svg width="430" height="180" style="background:#1e293b;border-radius:8px">{bars}</svg>
<p style="padding:20px;color:#38bdf8">status: operational | port: 9158</p></body></html>"""

if USE_FASTAPI:
    app = FastAPI(title="Adaptive Noise Injection")
