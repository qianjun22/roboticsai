"""
DAgger run191 symmetry-augmented planner service.
...
PORT = 10302
SERVICE = "dagger_run191_planner"
DESCRIPTION = "DAgger run191: symmetry-augmented DAgger — 3x free data from geometry, +3% SR"
...
<div class="metric"><h2>Service Info</h2><p>Port: {PORT} | Status: operational</p></div>
    ...
    @app.get("/dagger/run191/config")
    def run191_config():
        return {
            "run": 191,
            "strategy": "symmetry_augmented_dagger",
            "symmetry_transforms": ["lr_mirror", "rot_180", "flip_v"],
            "effective_multiplier": 4,
            "sr_target": 0.95
        }

    @app.get("/dagger/run191/metrics")
    def run191_metrics():
        return {
            "corrections_real": 0,
            "corrections_augmented": 0,
            "augmentation_factor": 4.0,
            "current_sr": 0.0,
            "status": "planned"
        }
