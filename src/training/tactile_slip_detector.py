"""Detect incipient slip before object drops — 80ms early warning, 95% no-drop rate
...
PORT = 10316
SERVICE = "tactile_slip_detector"
DESCRIPTION = "Detect incipient slip before object drops — 80ms early warning, 95% no-drop rate"
...
<div class="metric"><h2>Service Info</h2><p>Port: {PORT} | Status: operational</p></div>
    ...
    @app.post("/perception/slip/detect")
    def slip_detect():
        return {
            "slip_probability": round(random.uniform(0, 0.3), 3),
            "time_to_drop_ms": round(random.uniform(60, 120), 1),
            "correction_action": "increase_grip_force",
            "detection_latency_ms": 5.1
        }

    @app.get("/perception/slip/stats")
    def slip_stats():
        return {
            "no_drop_rate": 0.95,
            "avg_correction_ms": 17.0,
            "fragile_success_rate": 0.97,
