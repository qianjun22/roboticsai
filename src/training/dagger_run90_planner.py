"""DAgger Run90 Planner — FastAPI port 9898"""
import math, random
...
def build_html():
    ...
    return f"""<!DOCTYPE html><html><head><title>DAgger Run90 Planner — Port {PORT}</title>
<style>body{{margin:0;background:#0f172a;color:#e2e8f0;font-family:monospace}}
h1{{color:#C74634;padding:20px}}svg{{display:block;margin:20px}}</style></head>
<body><h1>DAgger Run90 Planner — Port {PORT}</h1>
...
if USE_FASTAPI:
    app = FastAPI(title="DAgger Run90 Planner")
