"""Hyperparam Transfer Learning — FastAPI port 8998"""
import math, random
...
def build_html():
    ...
    return f"""<!DOCTYPE html><html><head><title>Hyperparam Transfer Learning</title>
...
<body><h1>Hyperparam Transfer Learning</h1>
<div class="card"><h2>Transfer Metrics</h2>
<svg width="450" height="180">{bars}</svg>
<p>Current value: {data[-1]} | Peak: {max(data)} | Port: {PORT}</p>
</div></body></html>"""

if USE_FASTAPI:
    app = FastAPI(title="Hyperparam Transfer Learning")
