"""Revenue Retention Cohort — FastAPI port 9149"""
import math, random
...
def build_html():
    ...
    return f"""<!DOCTYPE html><html><head><title>Revenue Retention Cohort — Port 9149</title>
<style>body{{margin:0;background:#0f172a;color:#e2e8f0;font-family:monospace}}
h1{{color:#C74634;padding:20px}}svg{{display:block;margin:20px}}</style></head>
<body><h1>Revenue Retention Cohort — Port 9149</h1>
<svg width="430" height="180" style="background:#1e293b;border-radius:8px">{bars}</svg>
<p style="padding:20px;color:#38bdf8">status: operational | port: 9149</p></body></html>"""

if USE_FASTAPI:
    app = FastAPI(title="Revenue Retention Cohort")
