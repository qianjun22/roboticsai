"""ARR Milestone Tracker — FastAPI port 8993"""
import math, random
...
def build_html():
    ...
    return f"""<!DOCTYPE html><html><head><title>ARR Milestone Tracker</title>
...
<body><h1>ARR Milestone Tracker</h1>
<div class="card"><h2>Revenue Milestones</h2>
<svg width="450" height="180">{bars}</svg>
<p>Current value: {data[-1]} | Peak: {max(data)} | Port: {PORT}</p>
</div></body></html>"""

if USE_FASTAPI:
    app = FastAPI(title="ARR Milestone Tracker")
