...
PORT = 29381
SERVICE = "robotics_supportability_v4"
DESCRIPTION = "supportability v4 go-to-market platform"
...
def dashboard():
    ...
    return f"""<!DOCTYPE html><html><head><title>robotics_supportability_v4</title><style>body{margin:0;background:#0f172a;color:#e2e8f0;font-family:system-ui;}.header{background:#C74634;padding:20px 32px;}h1{margin:0;font-size:24px;color:#fff;}.sub{color:#fca5a5;font-size:13px;}.content{padding:32px;}.metric{background:#1e293b;border-radius:12px;padding:20px;}.bars{display:flex;gap:4px;align-items:flex-end;height:60px;}.bar{width:20px;border-radius:4px 4px 0 0;}</style></head><body><div class=\"header\"><h1>{service}</h1><div class=\"sub\">supportability v4 go-to-market platform</div></div><div class=\"content\"><div class=\"metric\"><b>Port:</b> {port} | <b>Status:</b> <span style=\"color:#4ade80\">&#x25cf; Live</span><div class=\"bars\">{bars}</div></div></div></body></html>"""
