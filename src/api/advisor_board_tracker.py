"""Advisor Board Tracker — FastAPI port 9333"""
import math, random
...
def build_html():
    ...
    return f"""<!DOCTYPE html><html><head><title>Advisor Board Tracker — Port {PORT}</title>
<style>body{{margin:0;background:#0f172a;color:#e2e8f0;font-family:monospace}}
h1{{color:#C74634;padding:20px}}svg{{display:block;margin:20px}}</style></head>
<body><h1>Advisor Board Tracker — Port {PORT}</h1>
...
if USE_FASTAPI:
    app = FastAPI(title="Advisor Board Tracker")
