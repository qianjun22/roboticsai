"""
Retention analytics dashboard service.
...
PORT = 10303
SERVICE = "retention_analytics_dashboard"
DESCRIPTION = "Retention analytics — 100% logo retention, NRR 118%, cohort breakdown"
...
<div class="metric"><h2>Service Info</h2><p>Port: {PORT} | Status: operational</p></div>
    ...
    @app.get("/retention/analytics/dashboard")
    def retention_dashboard():
        return {
            "logo_retention_pct": 100.0,
            "grr_pct": 100.0,
            "nrr_pct": 118.0,
            "net_expansion_k": 22,
            "cohorts": [
                {"name": "feb_machina", "age_mo": 2, "retention": 1.0},
                {"name": "mar_verdant_helix", "age_mo": 1, "retention": 1.0}
            ]
        }

    @app.get("/retention/analytics/risk")
    def retention_risk():
        return {
            "at_risk_customers": ["Helix"],
            "risk_factors": {
                "Helix": ["api_usage_down", "nps_41"]
            },
            "estimated_arr_at_risk_k": 33.6
        }
