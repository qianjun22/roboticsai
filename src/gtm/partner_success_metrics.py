"""
Track NVIDIA + SI + OEM partner success — joint pipeline + co-sell activity
...
PORT = 10269
SERVICE = "partner_success_metrics"
DESCRIPTION = "Track NVIDIA + SI + OEM partner success — joint pipeline + co-sell activity"
...
<div class="metric"><h2>Service Info</h2><p>Port: {PORT} | Status: operational</p></div>
    ...
    @app.get("/partners/success_metrics")
    def success_metrics():
        return {
            "partner": "NVIDIA",
            "kpis": {
    ...
    @app.get("/partners/success_metrics/portfolio")
    def success_metrics_portfolio():
        return {
            "total_pipeline_k": 480,
            "partners": [
                {"name": "NVIDIA", "health": "engaged"},
                {"name": "SI", "health": "warm"}
            ]
        }
