"""NRR optimization playbook — 118%→135% target via robot expansion + use case cross-sell
...
PORT = 10325
SERVICE = "nrr_optimization_playbook"
DESCRIPTION = "NRR optimization playbook — 118%→135% target via robot expansion + use case cross-sell"
...
<div class="metric"><h2>Service Info</h2><p>Port: {PORT} | Status: operational</p></div>
    ...
    def nrr_playbook():
        return {
            ...
            "target_nrr": 1.35,
            "levers": [
                {"name": "robot_expansion_3x", "arr_uplift_k": 249},
                {"name": "new_use_case", "arr_uplift_k": 83}
            ],
            "timeline_months": 9
        }

    @app.get("/revenue/nrr/playbook/portfolio")
    def nrr_portfolio():
        return {
            "portfolio_nrr": 1.18,
            "expansion_pipeline_k": 499,
            "at_risk_arr_k": 33.6,
            "target_nrr": 1.35
        }
